...
async def get_user_profile_name(user_id: int) -> str | None:
    async with SessionLocal() as session:
        row = await session.get(UserSettings, user_id)
        if row is None or not row.profile_name:
            return None
        return row.profile_name.strip() or None


async def set_user_profile_name(user_id: int, profile_name: str) -> str:
